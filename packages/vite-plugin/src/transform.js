 ...
 */
export function transformAstroToJs(ast, options) {
  return safeExecute(
...
function transformAstroToJsInternal(ast, options) {
  const {
    filename,
    dev = false,
    prettyPrint = true,
  ...
  // Generate the module
  const parts = [];
  const frontmatterImports = [];
  const componentImports = new Map(); // component name -> import path
  let frontmatterCode = '';
 ...
 */
function generateSourceMap(code, filename, ast) {
  const lines = code.split('\n');
 ...
 */
function encodeVLQ(generatedColumn, sourceIndex, sourceLine, sourceColumn) {
 ...
 */
export function extractClientScript(ast, options = {}) {
 ...
 */
export function hasClientDirectives(ast) {
  return checkNodeForClientDirectives(ast);
}
function checkNodeForClientDirectives(node) {
  switch (node.type) {
    case 'Fragment':
      return node.children.some(checkNodeForClientDirectives);
    case 'Element':
    case 'Component': {
      const element = node;
      return (
