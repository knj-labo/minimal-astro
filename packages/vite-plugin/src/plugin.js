...
import { analyzeAstForHmr, createErrorOverlay, handleAstroHmr, handleCssUpdate } from './hmr.js';
...
function createPluginCache(maxAge = 5 * 60 * 1000) {
  ...
  return {
    ...
    setAst(id, code, ast, diagnostics) {
      const hash = quickHash(code);
      astCache.set(id, {
...
export function astroVitePlugin(options = {}) {
  const opts = { ...DEFAULT_OPTIONS, ...options };
  // Store HMR state for each file
  const hmrStateMap = new Map();
  // Initialize cache
  ...
  function extractDependencies(ast) {
    ...
    const astObj = ast;
    if (astObj.children) {
      const frontmatter = astObj.children.find((child) => {
  ...
  return {
    ...
    configureServer(server) {
      ...
      const astroDevHandler = async (req, res, next) => {
        const url = req.url ?? '/';
        ...
        try {
          ...
          logger.error('Error rendering page', error, { url });
          next(error);
        }
      };
