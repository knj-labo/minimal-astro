 ...
 */
export function createJSXTransformer(options = {}) {
   ...
   */
  function transformFragment(node, context) {
    const children = node.children.map((child) => transformNode(child, context)).filter(Boolean);
...
${indent}</${opts.fragmentName}>`;
  ...
  function transformElement(node, context) {
    ...
    const indent = getIndent(context.depth);
    // Build props object - filter out null/undefined values
    ...
    const props = buildProps(filteredAttrs);
    // Transform children
    context.depth++;
  ...
  function transformComponent(node, context) {
    ...
    // Build props - filter out null/undefined values
    const filteredAttrs = regularAttrs
      .filter((attr) => attr.value !== null && attr.value !== undefined)
      .map((attr) => ({
        name: attr.name,
        value: attr.value,
    ...
    if (clientDirective) {
      ...
      const directiveValue = clientDirective.value || undefined;
   ...
   */
  function wrapWithHydration(componentName, props, directive, directiveValue, children, context) {
    // For now, add a data attribute to mark for hydration
    const hydrationProps = `data-astro-hydrate="${directive}"${directiveValue ? ` data-astro-hydrate-value="${directiveValue}"` : ''}`;
...
${indent}</div>`;
  ...
  function generateImports(context) {
    ...
    if (opts.useNewTransform) {
      imports.push(
        `import { jsx as _jsx, jsxs as _jsxs${context.imports.has('Fragment') ? ', Fragment' : ''} } from '${opts.jsxImportSource}/jsx-runtime';`
      );
    } else {
      imports.push(
        `import React${context.imports.has('Fragment') ? ', { Fragment }' : ''} from '${opts.jsxImportSource}';`
      );
    }
   ...
   */
  function wrapInComponent(jsxCode, _context) {
   ...
   */
  function extractVariables(code, context) {
    // Simple regex to find potential variable names
    const varPattern = /\b([a-zA-Z_$][a-zA-Z0-9_$]*)\b/g;
    let match = varPattern.exec(code);
    while (match) {
   ...
   */
  function isKeywordOrGlobal(name) {
    const keywords = [
      'true',
      'false',
      'null',
 ...
 */
export function astToJSX(ast, options) {
