...
function createInitialState(tokens, options = {}) {
  return {
    tokens,
    current: 0,
    diagnostics: [],
...
function addDiagnostic(state, code, message, loc, severity = 'error') {
  return {
    ...state,
    diagnostics: [
      ...state.diagnostics,
...
function parseChildren(state, parentTag) {
  const children = [];
...
export function parseAstro(source, options) {
  return safeExecute(
    () => {
