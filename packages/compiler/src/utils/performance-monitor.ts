...
function generateSuggestions(
  ...
  if (averages.parse > 50) {
    suggestions.push('Consider simplifying .astro file structure to improve parse times');
  }

  // Check for slow transformations
  if (averages.transform > 100) {
    suggestions.push('Transform operations are slow - consider reducing component complexity');
  }
  ...
  const totalOperations = Array.from(totals.values()).reduce((a, b) => a + b, 0);
  if (totalOperations > 5000) {
    suggestions.push('High total operation time - consider using object pooling');
  }
...
export function timed(operation?: string) {
  return function decorator(_target: unknown, propertyKey: string, descriptor: PropertyDescriptor) {
    const originalMethod = descriptor.value;
    const operationName = operation ?? `${String(propertyKey)}`;

    descriptor.value = function (this: unknown, ...args: unknown[]) {
      return globalPerformanceMonitor.time(operationName, () => originalMethod.apply(this, args), {
        method: propertyKey,
        args: args.length,
      });
    };
...
export function withTiming<T extends unknown[], R>(
  ...
  return (...args: T): R => {
    return globalPerformanceMonitor.time(operation, () => fn(...args), { args: args.length });
  };
}
...
export function withAsyncTiming<T extends unknown[], R>(
  ...
  return async (...args: T): Promise<R> => {
    return globalPerformanceMonitor.timeAsync(operation, () => fn(...args), { args: args.length });
  };
}
...
export function createPerformanceBudget(budgets: Record<string, number>) {
  return {
    check(
      ...
      const violations: Array<{ operation: string; actual: number; budget: number }> = [];
