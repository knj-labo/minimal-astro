 ...
 */
export function safeExecute(fn, options, fallback) {
  try {
    return fn();
  } catch (error) {
    const diagnostic = {
      code: 'internal-error',
      message: `An internal error occurred during ${options.operation}: ${error instanceof Error ? error.message : String(error)}`,
      severity: 'error',
      loc: {
