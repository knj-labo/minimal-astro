...
export function createSvelteRenderer(options: SvelteRendererOptions) {
	...
	function extractClientDirective(
		...
		const directiveType = clientAttr.name.slice(
			7,
		) as SvelteClientDirective["type"];
	...
	return {
		render(ast: FragmentNode): SvelteRenderResult {
			...
			const scripts = hydrate
				? generateSvelteHydrationScripts(hydrationData)
				: undefined;
...
function processFrontmatter(code: string): Record<string, unknown> {
	...
	try {
		...
		const logger = createContextualLogger({
			module: "svelte-frontmatter-processor",
		});
 ...
 * Create SSR renderer
 */
export function createSvelteSSRRenderer(
	options: Omit<SvelteRendererOptions, "mode"> = {},
) {
 ...
 */
export function createSvelteClientRenderer(
	options: Omit<SvelteRendererOptions, "mode"> = {},
) {
