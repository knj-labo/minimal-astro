...
function parseAttributes(state: ParserState): [ParserState, Attr[]] {
	...
	while (!isAtEnd(currentState)) {
		...
		if (
			token.type === TokenType.TagClose ||
			token.type === TokenType.TagSelfClose
		) {
			break;
		}
...
function parseExpression(
	state: ParserState,
): [ParserState, ExpressionNode | null] {
	const token = peek(state);
...
function parseChildren(
	...
	while (!isAtEnd(currentState)) {
		...
		if (
			token.type === TokenType.TagOpen &&
			implicitlyClosedBy.includes(token.value)
		) {
...
function parseElement(
	...
	if (closeToken) {
		if (closeToken.type === TokenType.TagSelfClose) {
			...
			if (!isVoidElement(tag)) {
				const [childrenState, parsedChildren] = parseChildren(
					currentState,
					tag,
				);
	...
	const endLoc =
		currentState.tokens[currentState.current - 1]?.loc.end ?? openToken.loc.end;
