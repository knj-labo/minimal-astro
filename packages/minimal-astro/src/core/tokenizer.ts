...
function consumeWhile(
	...
	while (
		currentState.position < currentState.source.length &&
		predicate(peek(currentState))
	) {
	...
	return [
		currentState,
		currentState.source.slice(start, currentState.position),
	];
}
	...
	if (isAtStart(state) && peekSequence(state, "---")) {
		...
		while (currentState.position < currentState.source.length) {
			if (peek(currentState) === "\n" && peekSequence(currentState, "\n---")) {
				const content = currentState.source.slice(
					contentStart,
...
function scanTag(state: TokenizerState): [TokenizerState, Token | null] {
	...
	if (peek(state) === "<") {
		...
		if (peek(currentState) === "/") {
			currentState = advance(currentState);
			const [newState, name] = consumeWhile(currentState, (c) =>
				/[a-zA-Z0-9-]/.test(c),
			);
			currentState = newState;

			const [spacesState, _] = consumeWhile(
				currentState,
				(c) => c === " " || c === "\t",
			);
...
function scanAttribute(state: TokenizerState): [TokenizerState, Token | null] {
	...
	// Scan attribute name
	const [nameState, name] = consumeWhile(currentState, (c) =>
		/[a-zA-Z0-9-:@]/.test(c),
	);
	if (name) {
		...
		const [spacesState2, _2] = consumeWhile(
			currentState,
			(c) => c === " " || c === "\t",
		);
		...
		if (peek(currentState) === "=") {
			currentState = advance(currentState);
			const [spacesState3, _3] = consumeWhile(
				currentState,
				(c) => c === " " || c === "\t",
			);
			...
			if (peek(currentState) === '"' || peek(currentState) === "'") {
				...
				currentState = advance(currentState);
				const [quotedState, quotedValue] = consumeWhile(
					currentState,
					(c) => c !== quote,
				);
