...
class ContentQueryImpl<T = Record<string, unknown>> implements ContentQuery<T> {
	...
	sort(
		compareFn: (a: ContentEntry<T>, b: ContentEntry<T>) => number,
	): ContentQuery<T> {
...
export function createContentManager(
	...
	const api: ContentAPI = {
		...
		async getEntry<T = Record<string, unknown>>(
			...
			return (
				(entries.find((entry) => entry.id === fullId) as ContentEntry<T>) ??
				null
			);
		},
		...
		async getEntryBySlug<T = Record<string, unknown>>(
			...
			const entries = await loadCollection(collection);
			return (
				(entries.find((entry) => entry.slug === slug) as ContentEntry<T>) ??
				null
			);
		},
...
export function getContentAPI(): ContentAPI {
	if (!globalContentAPI) {
		throw new Error(
			"Content API not initialized. Call initializeContentAPI first.",
		);
	}
	return globalContentAPI;
}
...
export const queries = {
	 ...
	 */
	byDateDesc<T = Record<string, unknown>>(
		a: ContentEntry<T>,
		b: ContentEntry<T>,
	): number {
		const dateA = new Date(
			(getDataProperty(a.data, "date") as string | Date) ?? 0,
		);
		const dateB = new Date(
			(getDataProperty(b.data, "date") as string | Date) ?? 0,
		);
	 ...
	 */
	byDateAsc<T = Record<string, unknown>>(
		a: ContentEntry<T>,
		b: ContentEntry<T>,
	): number {
		const dateA = new Date(
	 ...
	 * Sort by title
	 */
	byTitle<T = Record<string, unknown>>(
		a: ContentEntry<T>,
		b: ContentEntry<T>,
	): number {
