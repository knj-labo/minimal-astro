...
import { createVueSSRRenderer, renderVueComponent } from './vue-ssr.js';
 ...
 */
function detectFramework(componentName: string, options: UniversalSSROptions): FrameworkType {
...
export async function renderUniversalComponent(
  ...
  try {
    ...
    const actualFramework =
      framework === 'auto' ? detectFramework(componentName, options) : framework;
