...
function buildElementHtml(
	...
	if (hasOnlyTextChildren && options.prettyPrint) {
		...
		const childrenHtml = node.children
			.map((child) =>
				buildNodeHtml(child, { ...options, prettyPrint: false }, 0),
			)
			.join("");
...
export function createStreamingHtmlBuilder(options: HtmlBuilderOptions = {}) {
	...
	return {
		...
		buildToStream: async (
			...
			const buildFragmentToStream = async (
				node: FragmentNode,
				depth: number,
			): Promise<void> => {
			...
			const buildElementToStream = async (
				node: ElementNode,
				depth: number,
			): Promise<void> => {
				...
				await writeBuffered(
					`${indent}<${node.tag}${formatAttributes(node.attrs)}>`,
				);
			...
			const buildComponentToStream = async (
				node: ComponentNode,
				depth: number,
			): Promise<void> => {
				const indent = opts.prettyPrint
