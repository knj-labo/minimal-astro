...
export function astroVitePlugin(options: AstroVitePluginOptions = {}): Plugin {
	...
	function extractDependencies(ast: unknown): { all: string[]; css: string[] } {
		...
		if (astObj.children) {
			...
			if (frontmatter?.code) {
				const importMatches = frontmatter.code.match(
					/import\s+[^'"]+['"]([^'"]+)['"];?/g,
				);
				if (importMatches) {
	...
	return {
		...
		transform(code, id) {
			...
			try {
				...
				const transformed = transformAstroToJs(
					parseResult.ast as FragmentNode,
					{
						filename: id,
						dev: opts.dev,
		...
		async handleHotUpdate(ctx) {
			...
			if (isCssUpdate) {
				...
				if (dependentAstroFiles.length > 0) {
					logger.info(
						`CSS change affects ${dependentAstroFiles.length} Astro files`,
						{
							cssFile: ctx.file,
			...
			try {
				...
				const newHmrState = analyzeAstForHmr(
					parseResult.ast as FragmentNode,
					ctx.file,
				);
