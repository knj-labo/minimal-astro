...
export function renderReactComponent(
	...
	if (!React || !ReactDOM) {
		throw new Error(
			"React and ReactDOM must be available globally for React hydration",
		);
	}
 ...
 */
export function renderPreactComponent(
	component: ComponentType,
	container: HTMLElement,
): void {
...
export function validateRuntimeAvailability(runtime: "react" | "preact"): void {
	if (runtime === "react" && !isReactAvailable()) {
		throw new Error(
			"React runtime selected but React/ReactDOM not available globally",
		);
	}

	if (runtime === "preact" && !isPreactAvailable()) {
		throw new Error(
			"Preact runtime selected but Preact not available globally",
		);
	}
}
