...
import { createDefaultRenderer, validateRuntimeAvailability } from './framework-renderers.js';
import {
  addPending,
  clearObservers,
  createInitialState,
...
export function createHydrationRuntime(
  ...
  if (hooks) {
    if (hooks.onBeforeHydrate) eventSystem.on('before-hydrate', hooks.onBeforeHydrate);
    if (hooks.onAfterHydrate) eventSystem.on('after-hydrate', hooks.onAfterHydrate);
    if (hooks.onHydrationError) eventSystem.on('hydration-error', hooks.onHydrationError);
 ...
 */
export function autoHydrate(options: HydrationOptions, hooks?: FrameworkHooks): void {
