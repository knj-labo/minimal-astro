...
export function createEventSystem(): {
  emit<K extends keyof HydrationEvents>(event: K, data: HydrationEvents[K]): void;
  on<K extends keyof HydrationEvents>(event: K, handler: (data: HydrationEvents[K]) => void): void;
  off<K extends keyof HydrationEvents>(event: K, handler: (data: HydrationEvents[K]) => void): void;
} {
      ...
      if (eventListeners) {
        for (const listener of eventListeners) {
          try {
            ...
            console.error(`Error in hydration event listener for ${String(event)}:`, error);
          }
        }
      }
    },
