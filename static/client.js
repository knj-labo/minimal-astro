...
function hydrateIslands() {
  ...
  islands.forEach((element) => {
    ...
    if (!name || !window.__ISLANDS__[name])
      return;
    ...
    try {
      const root = import_client.hydrateRoot(element, import_react.default.createElement(import_react.StrictMode, null, import_react.default.createElement(Component, props)));
...
function Counter({ initialCount = 0 }) {
  const [count, setCount] = import_react2.useState(initialCount);
